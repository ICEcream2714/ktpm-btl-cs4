...
Client interface được phát triển sử dụng React và TypeScript, bao gồm các thành phần chính sau:

1. **Market Data Dashboard**

   - Hiển thị status của kết nối thời gian thực
   - Bộ lọc dữ liệu theo loại thị trường (Currencies, Energy, Metals)
   - Thẻ thông tin chi tiết cho từng loại dữ liệu thị trường
   - Biểu đồ giá tùy chỉnh cho từng loại dữ liệu
   - Bảng dữ liệu chi tiết với khả năng lọc theo loại

   ![alt text](screen_shot_1.png)

2. **Các thành phần hiển thị giá**

   - **PriceChart**: Biểu đồ Canvas tương tác hiển thị biến động giá theo thời gian với hiệu ứng gradient màu dựa trên xu hướng (xanh lá khi tăng, đỏ khi giảm)
   - **PriceDisplay**: Hiển thị giá hiện tại, mức thay đổi cả số tuyệt đối và phần trăm, giá cao nhất/thấp nhất trong ngày
   - **PriceTable**: Bảng dữ liệu lịch sử giá với các chỉ báo trực quan về sự thay đổi
   - **MarketStatus**: Hiển thị trạng thái thị trường (mở cửa/đóng cửa) và trạng thái kết nối socket
     ![alt text](screen_shot_2.png)

3. **Tính năng real-time**

   - Kết nối socket.io để nhận cập nhật giá theo thời gian thực
   - Hiệu ứng trực quan khi có dữ liệu mới (highlighted rows, animation)
   - Chỉ báo trạng thái kết nối (connected, connecting, disconnected)
   - Khả năng subscribe/unsubscribe từ các luồng dữ liệu khác nhau

4. **Responsive Design**
   - Giao diện thích ứng với các kích thước màn hình khác nhau
   - Layout grid có khả năng điều chỉnh từ 1 đến 3 cột tùy theo kích thước màn hình
   - Khả năng sử dụng trên cả desktop và thiết bị di động

Giao diện client được thiết kế với tông màu phù hợp với các loại giá cả và sử dụng các chỉ báo màu sắc trực quan (xanh lá: tăng, đỏ: giảm) để người dùng dễ dàng nắm bắt xu hướng thị trường.
